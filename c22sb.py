...
def tempoTimer(i):
    if chart["tempo_list"][i]["tick"] == 0:
        ...
        chart["tempo_list"][i]["time"] = chart["tempo_list"][i-1]["time"] + (chart["tempo_list"][i]["tick"] - chart["tempo_list"][i-1]["tick"]) * chart["tempo_list"][i-1]["value"] / 1000000 / chart["time_base"]
...
def newTextEvent(time,text,color,eventid,times,easings):
    if times != []:
        ...
        while  times[-1]["states"] != [] and times[-1]["states"][-1]["time"] > time:
            ...
            tmptip = 1
        if tmptip == 1:
            times[-1]["states"] += [{'time': time - 0.00000000002,'opacity': 1}]
            times[-1]["states"] += [{'time': time - 0.00000000001,'opacity': 0,'destory':True}]
    newTextValue = {'time':time,'text':text,'color':color,'id':eventid}
    textEventBasic =storyboard["templates"]["eventText"]
    newText = dict(list(newTextValue.items()) + copy.deepcopy(list(textEventBasic.items())))
    timep = time
...
def newNoteController(noteEvents,id,y):
    newNoteCtrl = [{'note':id,'time':"intro:$note",'override_y':True,'y':edgelimiter(y)}]
...
def newScannerPositionEvent(events,noteEvents,page,pageID,startTime,endTime,startTick,endTick,direction,scale,offset):
    ...
    if startPos != edgelimiter(startPos):
        events += [{'time': startTime-1e-10,'override_scanline_pos': True,'scanline_pos':edgelimiter(startPos),'page':page}]
        startOut = startPos - edgelimiter(startPos)
        startOutTick = startTick + abs(startOut) / scale * pagelen # 结束出界的Tick
        startTime = ticktotime(startOutTick)
    events += [{'time': startTime-1e-11,'override_scanline_pos': True,'scanline_pos':edgelimiter(startPos),'page':page}]
    ## 检查页间变速
    # lastTempo = chart["tempo_list"][0]
    for thisTempo in chart["tempo_list"]:
        if thisTempo["tick"] == startTick:
            ...
            tmpPos = (thisTempo["tick"] - startTick) / pagelen * scale * direction + startPos
            events += [{'time': thisTempo["time"],'override_scanline_pos': True,'scanline_pos':tmpPos,'tempo':thisTempo}]
    ...
    if endPos != edgelimiter(endPos):
        ...
        while events[-1]["time"] > endOutTime:
            del event[-1]
        events += [{'time': endOutTime,'override_scanline_pos': True,'scanline_pos':edgelimiter(endPos)}]
    events += [{'time': endTime,'override_scanline_pos': False,'scanline_pos':edgelimiter(endPos)}]
...
def newscaleEvent(events,noteEvents,pageID,page):
    ...
    newScannerPositionEvent(events,noteEvents,page,pageID,startTime,endTime,page["start_tick"],page["end_tick"],direction,scale,offset)
...
def newDownNoteClick(note,posevents,sprites):
    ...
    if note["NoteDirection"] == 0:
        ...
        y1 = -420
    posevents += [{'id':"pos_down_"+str(noteID),'note':noteID,'time':"intro:$note",'opacity_multiplier':0}]
...
def showScanline(events,tick):
    ...
    events += [{'time':time + 1e-10,'scanline_opacity':events[-1]['scanline_opacity']}]
...
for events in chart["event_order_list"]:
    ...
    for event in events["event_list"]:
        if event["type"] == 8:
            ...
            newTextEvent(time,text,color,"textEvent_"+str(events["tick"]),textEventTimes,textEventEasings)
...
for pageID in range(0,len(chart["page_list"])):
    ...
    if hasstate(chart["page_list"][pageID],"PositionFunction"):
        if chart["page_list"][pageID]["PositionFunction"]["Type"] == 0 and \
            ...
            newscaleEvent(scannerPositionEvents,notePositionEvents,pageID,chart["page_list"][pageID])
...
if scannerColorEvents != []:
    storyboard["controllers"] += [{'states':scannerColorEvents+[{'time':0.0000000001+scannerColorEvents[-1]['time'],"scanline_color":None}]}]
...
outputChart = './out/StoryBoard.json'
outputStoryBoard = './out/Chart.json'
...
if isOR == 'y' or isOR == 'Y': 
    with open(outputChart, 'wt') as f:
        ...
        print("Succcessfully output StoryBoard file to:",outputChart)
    with open(outputStoryBoard, 'wt') as f:
        print(json.dumps(cleanedChart), file=f)
        print("Succcessfully output Chart file to:",outputStoryBoard)
else:
    print("!!!DID NOTHING!!!")
